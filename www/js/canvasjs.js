/**
* @preserve CanvasJS HTML5 & JavaScript Charts - v1.9.8 GA - http://canvasjs.com/
...
(function () {
	...
	var disableLnk = (window && window.location && window.location.href && window.location.href.indexOf && (window.location.href.indexOf("canvasjs.com") !== -1 || window.location.href.indexOf("fiddle") !== -1));
	...
	function Chart(containerId, options) {
		...
		_this.setMarkerPosition = function(x, y, ctx, markerSize)
		{
			_this.markerCallback(x, y, ctx, markerSize);
		};

		_this.markerCallback = function()
		{
		};

		_this.setMarkerCallback = function(func)
		{
			_this.markerCallback = func;
		};
	...
	Chart.prototype.clearTextCache = function () {
		clearTextCache();
	};
	...
	Chart.prototype._initialize = function () {
		...
		this.layoutManager = new LayoutManager(0, 0, this.width, this.height, window.appConfig.chartPadding !== undefined ? window.appConfig.chartPadding : 2);
	...
	Chart.prototype.renderLine = function (plotUnit) {
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (dataPoints.length > 0) {
				...
				for (i = dataPoints.length - 1; i >= 0; i--) {
					...
					var cMapX = window.candlestickMap[(dataPointX) + 'x'];
					if (cMapX) {
						x = cMapX;
					}
		...
		RenderHelper.drawMarkers(markers, this);
	...
	Chart.prototype.renderSpline = function (plotUnit) {
		...
		// calculate px distance for 1x
		var map = Object.keys(window.candlestickMap).filter(key => key.substr(key.length - 1, 1) == 'x');
		if (map.length > 2) {
			var refXPos = window.candlestickMap[map[0]];
			var spacing = refXPos - window.candlestickMap[map[1]];
			var refX = parseFloat(map[0].substr(0, map[0].length - 1));
		}
			...
			if (dataPoints.length > 0) {
				...
				for (i = 0; i < dataPoints.length; i++) {
					...
					if (cMapX) {
						...
						x = (refXPos + (dataPointX - refX) * spacing);
					}
	...
	var drawRect = function (ctx, x1, y1, x2, y2, color, borderThickness, borderColor, top, bottom, left, right, fillOpacity, cornerRadius) {
		...
		var useRoundRect = window.appConfig && window.appConfig.roundedCandles;
		cornerRadius = cornerRadius || Math.min(window.appConfig.roundedCandles, Math.max(0, Math.abs(y1 - y2) - 1));

		if (useRoundRect) {
			roundRect(ctx, x1 - 1, y1, x2 - x1 + 2, y2 - y1, cornerRadius, true, false);
	...
	Chart.prototype.renderColumn = function (plotUnit) {
		...
		var yZeroToPixel = plotUnit.axisY.convertValueToPixel(plotUnit.axisY.viewportMinimum);
		...
		barWidth = window.appConfig.volumeBarWidth || 3;
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (dataPoints.length > 0) {
				...
				for (i = dataPoints.length - 1; i >= 0; i--) {
					...
					drawRect(ctx, x1, y1, x2, y2, color, 0, null, bevelEnabled, bevelEnabled, false, false, dataSeries.fillOpacity, {tl: 2, tr: 2});
	...
	Chart.prototype.renderSplineArea = function (plotUnit) {
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			var isRates = dataSeries.name == 'Rates';

			// determine the optimal spacing between candles
			if (isRates) {
				var prevSpacingX = 0;
				for (i = 0; i < Math.min(10, dataPoints.length); i++) {
					dataPointX = dataPoints[i].x;

					// consider round X values only
					if (dataPointX % 1) {
						continue;
					}

					var x = Math.round(plotUnit.axisX.convertValueToPixel(dataPointX));

					if (prevSpacingX) {
						Chart.prototype._candle_spacing = Math.max(Chart.prototype._candle_spacing, x - prevSpacingX);
					}
					prevSpacingX = x;
				}
			}

			var spacing = Chart.prototype._candle_spacing;
			var xDiff = 0;
			...
			if (dataPoints.length > 0) {
				...
				var gradientColors = plotUnit.axisY.chart.gradient;
				if (gradientColors) {
					var chartGradient = ctx.createLinearGradient(0, 0, 0, plotArea.height);
					chartGradient.addColorStop(0, gradientColors[0]);
					chartGradient.addColorStop(1, gradientColors[1]);
				...
				for (i = dataPoints.length - 1; i >= 0; i--) {
					...
					var dec = dataPoints[i].decBase || 0;
					...
					if (!dec) {
						...
						if (prevX !== undefined) {
							...
							if (!window.candlestickMap) {
								window.candlestickMap = {};
							}

							if (!window.candlestickMap[dataPointX]) {
								window.candlestickMap[dataPointX] = plotUnit.axisX.convertPixelToValue(x);
								window.candlestickMap[dataPointX + 'x'] = x;
							}
						}

						if (window.candlestickMap && window.candlestickMap[dataPointX + 'x']) {
							x = window.candlestickMap[dataPointX + 'x'];
						}

						if (x < 0 && prevX > 0 && isRates) {
							this.firstVisibleCandle = dataPoints[i + 1];
							this.firstVisibleCandleX = prevX;

							if (!this.visibleCandleCount) {
								this.visibleCandleCount = vi;
							}
						}

						prevX = x;
					} else {
						var prevRoundX = dataPoints[i].nextFullX;
						var prevPrevRoundX = dataPoints[i].nextFullX + 1;
						var mapX = window.candlestickMap[prevRoundX + 'x'];

						if (window.candlestickMap && mapX) {
							if (!xDiff) {
								var prev = window.candlestickMap[prevPrevRoundX + 'x'];
								var xDiff = prev - mapX;
							}

							x = mapX - ((xDiff || spacing) * dec);
						} else {
							// most likely last point(s) at the right side, should be accurate anyway without caching
							// console.log(x, dataPointX, i, dataPoints.length, xDiff, JSON.parse(JSON.stringify(candlestickMap)));
						}
					}
	...
	Chart.prototype.renderCandlestick = function (plotUnit) {
		...
		var minBarWidth = this.dataPointMinWidth ? this.dataPointMinWidth : this.dataPointWidth ? this.dataPointWidth : 1;
		var maxBarWidth = this.dataPointMaxWidth ? this.dataPointMaxWidth : this.dataPointWidth ? this.dataPointWidth : (this.width * .012);
		...
		var dpWidth = window.appConfig.candleWidth || this.dataPointWidth;

		var barWidth = dpWidth ? dpWidth : (plotArea.width * (plotUnit.axisX.logarithmic ? Math.log(xMinDiff) / Math.log(plotUnit.axisX.range) : Math.abs(xMinDiff) / Math.abs(plotUnit.axisX.range)) * .7) << 0;
		...
		if (!Chart.prototype._maxBarWidth || (barWidth > Chart.prototype._maxBarWidth)) {
			Chart.prototype._maxBarWidth = barWidth;
		}

		if (barWidth < Chart.prototype._maxBarWidth) {
			barWidth = Chart.prototype._maxBarWidth;
		}
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (dataSeries.type === "candlestick" && dataSeries.name == 'Rates') {
				if (ctx.canvas.scrollWidth !== Chart.prototype._last_canvas_width) {
					Chart.prototype._candle_spacing = 0;
					console.log('reset candle spacing');
				}

				if (!Chart.prototype._candle_spacing) {
					Chart.prototype._candle_spacing = 0;
				}

				Chart.prototype._last_canvas_width = ctx.canvas.scrollWidth;
			}
			...
			if (dataPoints.length > 0) {
				...
				// determine the optimal spacing between candles
				if (dataSeries.type === "candlestick" && isRates) {
					var prevSpacingX = 0;
					for (i = 0; i < Math.min(10, dataPoints.length); i++) {
						dataPoints[i].getTime ? dataPointX = dataPoints[i].x.getTime() : dataPointX = dataPoints[i].x;
						var x = Math.round(plotUnit.axisX.convertValueToPixel(dataPointX));
				...
				for (i = dataPoints.length - 1; i >= 0; i--) {
					...
					x = Math.floor(plotUnit.axisX.convertValueToPixel(dataPointX));
					...
					if (window.candlestickMap && window.candlestickMap[dataPointX + 'x'] && !isRates) {
						x = window.candlestickMap[dataPointX + 'x'];
					}
					...
					if (x < 0 && prevX > 0 && isRates) {
						...
						if (!this.visibleCandleCount) {
							this.visibleCandleCount = dataPoints.length - i + 1;
						}
					}
					...
					var id = dataSeries.dataPointIds[i];
					this._eventManager.objectMap[id] = {
						id: id, objectType: "dataPoint", dataSeriesIndex: dataSeriesIndex, dataPointIndex: i, x1: Math.floor(x1), y1: y1, x2: Math.floor(x2), y2: y2,
						x3: Math.floor(x), y3: y3, x4: Math.floor(x), y4: y4, borderThickness: borderThickness, color: color
					};

					var candleColor = dataPoints[i].y[0] <= dataPoints[i].y[3] ? dataSeries.risingColor : color;
					...
					if (dataSeries.type === "candlestick") {
						...
						drawRect(ctx, x1, Math.min(y1, y4), x2, Math.max(y1, y4), candleColor, borderThickness, candleColor, bevelEnabled, bevelEnabled, false, false, dataSeries.fillOpacity);
						...
						if (isCanvasSupported) {
							...
							ghostCtx.stroke();
							drawRect(ghostCtx, x1, Math.min(y1, y4), x2, Math.max(y1, y4), candleColor, 0, null, false, false, false, false);
						}
					}
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (dataPoints.length > 0) {
				...
				for (i = 0; i < dataPoints.length; i++) {
					...
					if (typeof (dataPoints[i].y[0]) !== "number" || typeof (dataPoints[i].y[1]) !== "number") {
						if (i > 0 && !prevDataNull) {
							if (dataSeries.connectNullData) {
	...
	function getCacheCanvas(textBlock)
	{
		var canvas = document.createElement('canvas');
		...
		//~ textBlock.ctx.canvas.parentNode.parentNode.appendChild(canvas);
	...
	var cachedText = null;
	var resets = 0;
	var TextCache = function(textBlock)
	{
		if (!cachedText)
		{
			cachedText = getCacheCanvas(textBlock);
			cachedText.clearRect(0, 0, cachedText.canvas.width, cachedText.canvas.height);
		}

		cachedText.textBaseline = 'middle';
		var size = textBlock._wrappedText.lines[0];
		var id = textBlock._wrappedText.lines[0].text + '';
		...
		else
		{
			...
			cacheTop += (textBlock._wrappedText.lines[0].height) + 5;
			cacheIdx[id] = cacheTop;
		}

		return {
			originalCtx: textBlock.ctx,
			isCached: isCached,
			ctx: cachedText,
			top: cacheTop,
			restore: function(x, y)
			{
				sourceCtx = cachedText;
				destinationCtx = textBlock.ctx;

				var offset = textBlock._wrappedText.lines[0].height / 2;
				var top = cacheIdx[id] - offset - 1;

				imageData = sourceCtx.getImageData(0, top * devicePixelBackingStoreRatio, Math.ceil(textBlock._wrappedText.lines[0].width) * devicePixelBackingStoreRatio, textBlock._wrappedText.lines[0].height * devicePixelBackingStoreRatio);
				destinationCtx.putImageData(imageData, Math.round(x * devicePixelBackingStoreRatio), Math.round((y - offset) * devicePixelBackingStoreRatio));
			}
		};
	};
	...
	TextBlock.prototype.render = function (preserveContext) {
		...
		if (this._wrappedText.lines[0])
		{
			var cache = TextCache(this);
			if (!cache.isCached)
			{
				this.ctx = cache.ctx;
				...
				this.ctx.translate(0, cache.top);
				...
				for (var i = 0; i < this._wrappedText.lines.length; i++) {
					...
					// there was something with ios
					//~ this.ctx.fillStyle = '#fff';
					window.renderChartLabel(line.text, textLeft, textTop, this.x, line.width);
				...
				this.ctx = cache.originalCtx;
			}

			this.ctx.translate(this.x, this.y + offsetY);

			cache.restore(this.x, this.y + offsetY);
		}
	...
	// CanvasJS original - restored because of Android WebView issues
	TextBlock.prototype.render = function (preserveContext) {
		if (preserveContext)
			this.ctx.save();
		...
		var isX = !!this.options.cornerRadius;
		...
		for (var i = 0; i < this._wrappedText.lines.length; i++) {
			...
			if (window.renderChartLabel) {
				if (isX) {
					textLeft = textLeft - (line.width / 2);
				}

				window.renderChartLabel(line.text.toUpperCase(), textLeft, textTop, this.x, line.width, this.ctx, isX);
			}
	...
	TextBlock.prototype._getLineWithWidth = function (text, width, clipWord) {
		...
		if (widthCache[text])
		{
			return {text: text, width: widthCache[text]};
		}
						...
						case "rangeBar":
							...
							if (x >= visualInfo.x1 && x <= visualInfo.x2 && y >= visualInfo.y1 && y <= visualInfo.y2) {
								results.push({
									...
									dataPointIndex: i,
									dataSeries: this,
									distance: Math.ceil(Math.min(Math.abs(visualInfo.x1 - x), Math.abs(visualInfo.x2 - x), Math.abs(visualInfo.y1 - y), Math.abs(visualInfo.y2 - y)))
									// distance:2
								});
	...
	Axis.prototype.createLabels = function () {
		...
		if (this.type === "axisX" && isDateTime && !this.logarithmic) {
			...
			var viewMax = this.viewportMaximum;
			endPoint = viewMax + (labelInterval * 5);
			this.viewportMaximum = endPoint;
			...
			if (this.labels) {
				...
				for (i = tempStartPoint; i < endPoint; i += tempInterval) {
					if (this.labels[i]) {
						hasAllLabels = true;
					} else {
						hasAllLabels = false;
			...
			var idx = 0;
			var cnt = 0;
			for (i = this.intervalStartPosition; i <= endPoint; i = parseFloat((this.logarithmic && this.equidistantInterval ? i * Math.pow(this.logarithmBase, this.interval) : i + this.interval).toFixed(14))) { cnt++; };
			...
			for (i = this.intervalStartPosition; i <= endPoint; i = parseFloat((this.logarithmic && this.equidistantInterval ? i * Math.pow(this.logarithmBase, this.interval) : i + this.interval).toFixed(14))) {
				var text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.options, value: i, label: this.labels[i] ? this.labels[i] : null, idx: idx++, cnt: cnt})
                    : this.type === "axisX" && this.labels[i] ? this.labels[i] : numberFormat(i, this.valueFormatString, this.chart._cultureInfo);
	...
	Axis.prototype.createLabelsAndCalculateHeight = function () {
		...
		if (this._position === "bottom" || this._position === "top") {
			...
			for (i = 0; i < this._labels.length; i++) {
				...
				var labelEffectiveHeight = size.height;
	...
	Axis.setLayoutAndRender = function (axisX, axisX2, axisY, axisY2, axisPlacement, freeSpace) {
		...
		if (axisPlacement === "normal") {
				...
				if (axisY2 && axisY2.length > 0) {
					for (var k = 0; k < axisY2.length; k++) {
						...
						if (axisY2[k]) {
							...
							//y2 = freeSpace.y2 - axisXHeight - axisX.margin - axisX.lineThickness / 2;
							if (!(axisX.length > 0)) {
								//~ padding = (axisY2[k]._labels[0].textBlock || {height: 0}).height / 2;
								padding = 0;
							} else {
								padding = 0;
							}
			...
			if (window.appConfig.drawGridLines) {
				window.appConfig.drawGridLines(chart, true);
			}
				...
				if (axisX2 && axisX2.length > 0) {
					for (var k = 0; k < axisX2.length; k++) {
						...
						if (axisY && axisY.length > 0) {
							for (var l = 0; l < axisY.length; l++) {
								if (axisY[l] && axisY[l].labelAutoFit) {
									x2 = axisY[l].labelAngle < 0 ? Math.max(x2, firstLabelWidthY) : axisY[k].labelAngle === 0 ? Math.max(x2, firstLabelWidthY / 2) : x2;
		...
		if (window.appConfig.drawGridLines) {
			window.appConfig.drawGridLines(chart);
		}
	...
	Axis.prototype.renderLabelsTicksAndTitle = function () {
		...
		var maxCnt = this._position === "bottom" ? 4 : 90;
		var labelOffset = this._position === "bottom" ? 0.5 : -1;
		while (this._labels.length > maxCnt)
		{
			this._labels.pop();
		}

		while (this._labels[0] && this._labels[0].textBlock.text.substr(0, 1) == '-')
		{
			this._labels.shift();
		}

		var spaceBetween = (this.viewportMaximum - this.viewportMinimum) / (this._labels.length);
		for (var k = 0; k < this._labels.length; k++)
		{
			var off = (k + labelOffset);
			this._labels[k].position = this.viewportMinimum + (off * spaceBetween);
		}
		...
		// push Y axis labels a bit up
		if (this._position !== "bottom")
		{
			var first = this._labels.shift();
			var last = this._labels[this._labels.length - 1];
			var slast = this._labels[this._labels.length - 2];
			...
			first.position = last.position + (last.position - slast.position);

			var lp = parseFloat(last.textBlock.text);
			var sp = parseFloat(slast.textBlock.text);
			var p = lp + (lp - sp);

			if (first.textBlock._wrappedText.lines[0])
			{
				first.textBlock._wrappedText.lines[0].text = (p.toString() + '0000').substr(0, last.textBlock.text.length);
			}
			this._labels.push(first);
		}
		...
		if (this._position === "bottom") {
			...
			var min = this.viewportMinimum
			var max = this.viewportMaximum;
			...
			for (i = 0; i < this._labels.length; i++) {
				...
				if (0 == i) {
					...
					label.position = min + (((max - min) / (c - 1)) * i);
				}

				xy = this.getPixelCoordinatesOnAxis(label.position);
				if (!window.CanvasJS.Chart.prototype._label_pos) {
					window.CanvasJS.Chart.prototype._label_pos = xy.y;
				}

				xy.y = window.CanvasJS.Chart.prototype._label_pos;
				...
				if (this._labels.length - 1 == i) {
					var end = this.getPixelCoordinatesOnAxis(max).x;
					// xy.x = end - (label.textBlock.width / 2);
				}

				xy.y += this.tickLength + label.textBlock.fontSize / 2;

				if (window.appConfig.xLabelPositionTransform) {
					xy = JSON.parse(JSON.stringify(xy));
					xy = window.appConfig.xLabelPositionTransform(label, xy, i, window.chart);
				}
			...
			for (var i = 1; i < this._labels.length; i++) {
				...
				window.labelCache.ySrc[i].pos = {x: label.textBlock.x, y: label.textBlock.y};
				window.labelCache.ySrc[i].height = label.textBlock.height;
				window.labelCache.ySrc[i].renderTime = Date.now();

				// when using number formatting with commas, for some reason the first and sometimes the last label values on Y axis are totally off (autocalculated by CanvasJS)
				// if (!label.textBlock._wrappedText.lines[0]) {
				// 	label.textBlock._wrappedText.lines.push({text: '0.0000'});
				// }

				if (label.textBlock._wrappedText.lines[0] && typeof label.textBlock._wrappedText.lines[0].text == 'string') {
					label.textBlock._wrappedText.lines[0].text = window.chartOpts.axisY2.labelFormatterCallback(label.position.toString(), {idx: i - 1, last: true, chart: window.chart, value: label.position, cnt: this._labels.length, pos: window.labelCache.ySrc[i].pos});
				}
	...
	Axis.prototype.convertValueToPixel = function (value) {
		...
		if (this.logarithmic)
			pixel = (this.conversionParameters.reference + this.conversionParameters.pixelPerUnit * Math.log(value / this.conversionParameters.minimum) / this.conversionParameters.lnLogarithmBase);
		else
			pixel = (this.conversionParameters.reference + this.conversionParameters.pixelPerUnit * (value - this.conversionParameters.minimum));
	...
	var RenderHelper = {
		drawMarker: function (x, y, ctx, markerType, markerSize, markerColor, markerBorderColor, markerBorderThickness, chart) {
			...
			if (markerType === "circle") {
				...
				if (chart && chart.setMarkerPosition)
				{
					chart.setMarkerPosition(x, y, ctx, markerSize);
				}
		...
		drawMarkers: function (markers, chart) {
			for (var i = 0; i < markers.length; i++) {
				var marker = markers[i];

				RenderHelper.drawMarker(marker.x, marker.y, marker.ctx, marker.type, marker.size, marker.color, marker.borderColor, marker.borderThickness, chart);
			}
		}
	...
	CanvasJS.Chart.version = "v1.9.8 GA";
